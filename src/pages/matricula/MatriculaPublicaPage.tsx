...
import { CheckCircle, Upload, X, MapPin, FileDown, AlertTriangle, MessageCircle, ArrowRight, Search } from "lucide-react";
...
const Field = ({
  label, value, onChange, required, type = "text", placeholder, onBlur,
}: {
  label: string; value: string; onChange: (val: string) => void;
  required?: boolean; type?: string; placeholder?: string; onBlur?: () => void;
}) => (
  <div>
    <Label className="text-sm font-medium">
      {label}{required && <span className="text-destructive ml-0.5">*</span>}
    </Label>
    <Input type={type} value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} className="mt-1" placeholder={placeholder} />
  </div>
);
...
const MatriculaPublicaPage = () => {
  ...
  const handleIdentifySubmit = async () => {
    ...
    const dob = form.data_nascimento;
    if (!nome) { toast.error("Informe o nome completo"); return; }
    if (!dob) { toast.error("Informe a data de nascimento"); return; }

    // Validate age
    const dobDate = new Date(dob);
    const now = new Date();
    const age = (now.getTime() - dobDate.getTime()) / (365.25 * 24 * 60 * 60 * 1000);
    if (dobDate > now) { toast.error("Data de nascimento não pode ser futura"); return; }
    if (age < 4 || age > 99) { toast.error("Idade fora da faixa atendida (4-99 anos)"); return; }

    setChecking(true);
    setFuzzyMatch(null);
    try {
      const nomePadronizado = stripAccents(nome.toUpperCase());
      set("nome_completo", nomePadronizado);

      const projectId = import.meta.env.VITE_SUPABASE_PROJECT_ID;
      const res = await fetch(`https://${projectId}.supabase.co/functions/v1/public-check-participante`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome_completo: nomePadronizado, data_nascimento: dob }),
      });
      ...
      if (data.found && data.participante) {
        ...
        if (data.match_type === "fuzzy") {
          // Show fuzzy suggestion — ask user to confirm
          setFuzzyMatch({ nome: p.nome_completo, similaridade: data.similaridade });
          setExistingId(p.id);
          // Pre-fill but don't advance yet
          prefillFromParticipant(p);
          setChecking(false);
          return;
        }
  ...
  const prefillFromParticipant = (p: any) => {
    setForm((prev) => ({
      ...prev,
    ...
    if (p.bairro_nome) loadPontos(p.bairro_nome);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const doc of docs) {
        if (doc.file.size > MAX_FILE_SIZE) { alert(`Arquivo "${doc.file.name}" excede 5MB e será ignorado.`); continue; }
        const base64 = await fileToBase64(doc.file);
        const ts = new Date().toISOString().replace(/[:.]/g, "-");
        docsPayload.push({
          base64, categoria: doc.categoria,
          ...
          <CardContent className="pt-8 pb-8 space-y-5">
            ...
            {whatsappLink && (
              <div className="space-y-2 pt-2">
                ...
                <Button className="w-full h-12 text-base font-semibold gap-2" style={{ backgroundColor: "#25D366", color: "#fff" }} asChild>
            ...
            <Button onClick={resetAll} variant="outline">Fazer outra matrícula</Button>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            ...
            <Button variant="outline" size="sm" className="mt-2 gap-2" asChild>
              <a href="https://drive.google.com/file/d/1SOjU9Rmy9vHl85bO4cBe6WNxVdy9NrTv/view" target="_blank" rel="noopener noreferrer">
        ...
        {step === "identify" && (
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Informe o nome e a data de nascimento para verificarmos se já existe cadastro.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <Field
                    ...
                    onChange={(v) => set("nome_completo", v)}
                    required
                  />
                </div>
                <div>
              ...
              {fuzzyMatch && (
                <Alert className="border-yellow-400 bg-yellow-50">
                  ...
                  <AlertDescription className="text-yellow-800 text-sm space-y-3">
                    <p>
                      Encontramos um cadastro com nome semelhante: <strong>{fuzzyMatch.nome}</strong>
                      <br />
                      <span className="text-xs text-yellow-600">({Math.round(fuzzyMatch.similaridade * 100)}% de similaridade)</span>
                    </p>
                    <p className="text-xs">É a mesma pessoa?</p>
                    <div className="flex gap-2">
                      <Button size="sm" onClick={confirmFuzzyMatch} className="text-xs h-8">
                        Sim, é a mesma pessoa
                      </Button>
                      <Button size="sm" variant="outline" onClick={rejectFuzzyMatch} className="text-xs h-8">
                        Não, é outra pessoa
                      </Button>
                    </div>
                  </AlertDescription>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="sm:col-span-2">
                    <Field label="Nome Completo" value={form.nome_completo || ""} onChange={(v) => set("nome_completo", v)} required />
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Data de Nascimento<span className="text-destructive ml-0.5">*</span></Label>
                    <Input type="date" value={form.data_nascimento || ""} onChange={(e) => set("data_nascimento", e.target.value)} className="mt-1" disabled />
                  </div>
                  <div>
                ...
                <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {form.bairro_scfv && (
                    <div className="sm:col-span-2">
                      ...
                      <a href={MAPS_LINK} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline mt-1.5">
                        <MapPin className="h-3 w-3" />Confira a localização dos pontos no mapa
                      </a>
                    </div>
                  )}
                </CardContent>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <Input value={maskPhone(form.responsavel1_whatsapp || "")} onChange={(e) => set("responsavel1_whatsapp", unmaskDigits(e.target.value))} className="mt-1" placeholder="(00) 00000-0000" />
                    </div>
                  </div>
                  <div className="border-t pt-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <Input value={maskPhone(form.responsavel2_whatsapp || "")} onChange={(e) => set("responsavel2_whatsapp", unmaskDigits(e.target.value))} className="mt-1" placeholder="(00) 00000-0000" />
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <p className="text-xs text-muted-foreground">Envie fotos ou PDFs dos documentos.</p>
                    ...
                    {DOC_CATEGORIES.map((cat) => {
                      ...
                      return (
                        <Button key={cat.value} type="button" variant="outline" size="sm" className="text-xs h-auto py-2 flex-col gap-1 relative" onClick={() => triggerUpload(cat.value)}>
                          <Upload className="h-4 w-4" />{cat.label}
                    ...
                    <div className="space-y-1.5">
                      {docs.map((doc, i) => (
                        <div key={i} className="flex items-center gap-2 bg-muted/50 rounded p-2 text-xs">
                          ...
                          <button type="button" onClick={() => removeDoc(i)} className="text-destructive hover:text-destructive/80"><X className="h-3.5 w-3.5" /></button>
                        </div>
                      ))}
                    </div>
                  )}
                  <input ref={fileRef} type="file" accept="image/*,application/pdf" className="hidden" onChange={handleFileSelected} />
                </CardContent>
              </Card>
