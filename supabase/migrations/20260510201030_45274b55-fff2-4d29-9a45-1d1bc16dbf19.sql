...
DECLARE
  ...
  v_ativos_mes_atual int;
  v_ativos_mes_anterior int;
  v_por_faixa jsonb;
  v_por_genero jsonb;
  v_por_bairro jsonb;
  v_por_periodo jsonb;
...
BEGIN
  ...
  -- NOVO: ativos com presença registrada (presente=true) no mês atual e no anterior
  SELECT count(DISTINCT rp.participante_id) INTO v_ativos_mes_atual
  FROM relatorio_presenca rp
  JOIN relatorios_atividade ra ON ra.id = rp.relatorio_id
  ...
  SELECT count(DISTINCT rp.participante_id) INTO v_ativos_mes_anterior
  FROM relatorio_presenca rp
  JOIN relatorios_atividade ra ON ra.id = rp.relatorio_id
  WHERE rp.presente = true
  ...
  v_delta_participantes := v_ativos_mes_atual - v_ativos_mes_anterior;
  ...
  result := jsonb_build_object(
    ...
    'participantesAtivosMesAtual', v_ativos_mes_atual,
    'participantesAtivosMesAnterior', v_ativos_mes_anterior,
    'deltaParticipantesBase', 'presenca_mensal',
