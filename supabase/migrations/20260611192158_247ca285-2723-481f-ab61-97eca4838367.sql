...
BEGIN
  ...
  -- Órfã: participante não está em NENHUMA das turmas listadas no relatório
  WITH rel_periodo AS (
    SELECT ra.id AS relatorio_id, ra.data
    FROM public.relatorios_atividade ra
    WHERE ra.data BETWEEN _de AND _ate
  ),
  rel_turmas AS (
    SELECT relatorio_id, array_agg(turma_id) AS turma_ids
    FROM public.relatorio_turmas
    WHERE relatorio_id IN (SELECT relatorio_id FROM rel_periodo)
    GROUP BY relatorio_id
  ),
  orfas AS (
    SELECT rp.id AS presenca_id, rp.relatorio_id, rp.participante_id, rp_data.data
    FROM public.relatorio_presenca rp
    JOIN rel_periodo rp_data ON rp_data.relatorio_id = rp.relatorio_id
    JOIN rel_turmas rt ON rt.relatorio_id = rp.relatorio_id
    WHERE rp.participante_id IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.turma_participantes tp
        WHERE tp.participante_id = rp.participante_id
          AND tp.turma_id = ANY(rt.turma_ids)
      )
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
