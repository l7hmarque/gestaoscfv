...
BEGIN
  ...
  -- Participantes Ativos: com filtro = distintos com pelo menos 1 presença no período
  ...
  -- Distribuições: com filtro, restritas aos frequentantes do período
  IF v_has_filter THEN
    ...
    FROM (
      SELECT
        CASE
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) BETWEEN 6 AND 8 THEN '6-8'
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) BETWEEN 9 AND 11 THEN '9-11'
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) BETWEEN 12 AND 17 THEN '12-17'
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) >= 60 THEN 'idosos'
          ELSE NULL
        END AS faixa,
        count(*) AS cnt
      FROM (
        SELECT DISTINCT rp.participante_id
        FROM relatorio_presenca rp
        JOIN relatorios_atividade ra ON ra.id = rp.relatorio_id
        WHERE rp.presente = true AND ra.data >= v_effective_start AND ra.data < v_filter_end
      ) f
      JOIN participantes p ON p.id = f.participante_id AND p.is_teste = false AND p.data_nascimento IS NOT NULL
      GROUP BY faixa
      HAVING CASE
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) BETWEEN 6 AND 8 THEN '6-8'
          WHEN extract(year FROM age(v_filter_end - 1, p.data_nascimento)) BETWEEN 9 AND 11 THEN '9-11'
    ...
    INTO v_por_genero
    FROM (
      SELECT coalesce(p.genero, 'Não informado') AS g, count(*) AS cnt
      FROM (
        SELECT DISTINCT rp.participante_id
        FROM relatorio_presenca rp
        JOIN relatorios_atividade ra ON ra.id = rp.relatorio_id
      ...
      JOIN participantes p ON p.id = f.participante_id AND p.is_teste = false
      GROUP BY g
    ) sub;
