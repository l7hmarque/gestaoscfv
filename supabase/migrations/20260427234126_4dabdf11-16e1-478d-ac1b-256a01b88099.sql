...
BEGIN
  ...
  -- Delta participantes ativos: respeita marco operacional e considera apenas quem
  -- estava efetivamente ativo/em busca ativa há 30 dias.
  SELECT count(*) INTO v_count_now 
  FROM participantes 
  WHERE status = 'ativo';

  IF v_data_30d < v_data_corte THEN
    -- Antes do marco operacional, não há histórico significativo: zera o delta.
