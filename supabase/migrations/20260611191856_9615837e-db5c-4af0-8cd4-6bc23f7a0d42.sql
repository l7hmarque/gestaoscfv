...
BEGIN
  -- 1) Pares (turma × data) com 2+ relatórios
  WITH pares AS (
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'turma_nome', t.nome,
    'data', p.data,
    'qtd_relatorios', p.qtd_relatorios,
    'relatorio_ids', p.relatorio_ids
  ) ORDER BY p.data DESC), '[]'::jsonb)
  INTO v_duplicados
  FROM pares p
  LEFT JOIN public.turmas t ON t.id = p.turma_id;

  -- 2) Participantes ativos na turma+data SEM registro de presença
  WITH rels AS (
  ...
  faltantes AS (
    ...
    LEFT JOIN public.relatorio_presenca rp
      ON rp.relatorio_id = e.relatorio_id
     AND rp.participante_id = e.participante_id
    WHERE rp.id IS NULL
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  -- 3) Presenças órfãs: registro existe mas participante não estava ativo na turma+data
  WITH rels AS (
    SELECT ra.id AS relatorio_id, ra.data, rt.turma_id
    FROM public.relatorios_atividade ra
    JOIN public.relatorio_turmas rt ON rt.relatorio_id = ra.id
  ...
  orfas AS (
    ...
    WHERE rp.participante_id IS NOT NULL
      AND NOT EXISTS (
        ...
        WHERE tp.turma_id = r.turma_id
          AND tp.participante_id = rp.participante_id
          AND tp.data_entrada <= r.data
          AND (tp.data_saida IS NULL OR tp.data_saida > r.data)
      )
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'presenca_id', o.presenca_id,
    'relatorio_id', o.relatorio_id,
    'data', o.data,
    'turma_id', o.turma_id,
    'turma_nome', t.nome,
  ...
  -- 4) Relatórios sem nenhuma turma vinculada
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'relatorio_id', ra.id,
    'data', ra.data,
    'nome_atividade', ra.nome_atividade
  ...
  -- 5) Relatórios sem nenhuma presença registrada
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'relatorio_id', ra.id,
    'data', ra.data,
    'nome_atividade', ra.nome_atividade
...
GRANT EXECUTE ON FUNCTION public.auditar_integridade_presencas(date, date) TO authenticated, service_role;
