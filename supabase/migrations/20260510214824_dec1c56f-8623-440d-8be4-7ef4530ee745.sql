...
BEGIN
  ...
  IF v_has_filter THEN
    SELECT coalesce(avg(score_elo), 0), count(*) FILTER (WHERE score_elo IS NOT NULL),
      ...
      FROM relatorios_atividade WHERE data >= v_effective_start AND data < v_filter_end;
    SELECT coalesce(avg(pct_adesao), 0) INTO v_media_adesao_consolidada
      FROM relatorios_atividade WHERE data >= v_effective_start AND data < v_filter_end
  ...
  SELECT coalesce(jsonb_object_agg(faixa, qtd), '{}'::jsonb) INTO v_por_faixa FROM (
    SELECT CASE
        ...
        ELSE '60+' END as faixa, count(*) as qtd
  ...
  SELECT coalesce(jsonb_object_agg(coalesce(b.nome,'N/I'), qtd), '{}'::jsonb) INTO v_por_bairro FROM (
    SELECT b.nome, count(*) as qtd FROM participantes p
      LEFT JOIN bairros b ON b.id = p.bairro_id
      WHERE p.status = 'ativo' AND p.is_teste = false GROUP BY b.nome
  ) b;
  ...
  -- competências: agrega contagens das 5 dimensões avaliadas (não nulas)
  ...
  WHERE data >= COALESCE(v_effective_start, v_data_corte)
    AND (NOT v_has_filter OR data < v_filter_end);

  -- objetivos alcançados (enum único objetivo_alcancado)
  SELECT coalesce(jsonb_object_agg(status, total), '{}'::jsonb) INTO v_objetivos FROM (
    SELECT objetivo_alcancado::text as status, count(*) as total
    FROM relatorios_atividade
    WHERE objetivo_alcancado IS NOT NULL
      AND data >= COALESCE(v_effective_start, v_data_corte)
      AND (NOT v_has_filter OR data < v_filter_end)
    GROUP BY 1
  ) sub;

  -- frequência geral usa num_matriculados como base planejada
