...
BEGIN
  ...
  -- Relatórios reais (excluindo sintéticos) e consolidados separados
  IF v_has_filter THEN
    SELECT 
  ...
  IF v_has_filter THEN
    ...
    FROM relatorios_atividade
    WHERE data >= v_filter_start AND data < v_filter_end;
  ...
  ELSE
    ...
    INTO v_media_adesao_consolidada
    FROM relatorios_atividade
    WHERE coalesce(nome_atividade,'') LIKE v_synthetic_filter;
  END IF;
  ...
  -- Adesão mensal (apenas relatórios reais)
  ...
  -- Top 5 educadores (excluindo sintéticos)
  IF v_has_filter THEN
  ...
  -- Presença mensal (com flag parcial)
  ...
  -- Delta participantes ativos: cadastros atuais vs 30 dias atrás
  ...
  -- Atividades recentes (últimos 5, EXCLUINDO sintéticos)
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'id', r.id,
