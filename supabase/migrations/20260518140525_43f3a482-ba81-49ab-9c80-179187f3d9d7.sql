...
BEGIN
  ...
  v_has_dim_filter := (_faixa IS NOT NULL OR _genero IS NOT NULL OR _bairro_id IS NOT NULL OR _periodo IS NOT NULL OR _idade_min IS NOT NULL OR _idade_max IS NOT NULL);

  IF v_has_dim_filter THEN
    SELECT coalesce(array_agg(p.id), ARRAY[]::uuid[])
      INTO v_pf_ids
    FROM participantes p
    WHERE (NOT _apenas_ativos OR p.status = 'ativo') AND p.is_teste = false
  ...
  WHERE rp.presente = true AND ra.data >= v_effective_start AND ra.data < v_filter_end
    AND (NOT _apenas_ativos OR p.status = 'ativo')
  ...
  SELECT coalesce(jsonb_object_agg(coalesce(genero,'N/I'), qtd), '{}'::jsonb) INTO v_por_genero FROM (
    ...
    WHERE (NOT _apenas_ativos OR status = 'ativo') AND is_teste = false
      AND (_bairro_id IS NULL OR p.bairro_id = _bairro_id)
      AND (_periodo IS NULL OR p.periodo::text = _periodo)
      AND (_idade_min IS NULL OR extract(year from age(current_date, p.data_nascimento)) >= _idade_min)
      AND (_idade_max IS NULL OR extract(year from age(current_date, p.data_nascimento)) <= _idade_max)
  ...
  SELECT count(*) INTO v_total_alerta FROM (
    ...
    FROM relatorio_presenca rp
    JOIN relatorios_atividade ra ON ra.id = rp.relatorio_id
    JOIN participantes p ON p.id = rp.participante_id AND (NOT _apenas_ativos OR p.status = 'ativo') AND p.is_teste = false
