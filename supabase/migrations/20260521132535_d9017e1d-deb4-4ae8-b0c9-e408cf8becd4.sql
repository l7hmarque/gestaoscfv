...
-- Reescreve get_participantes_turma com:
--  • parâmetro _modo ('frequencia' | 'chamada_branco')
--  • filtro temporal pelo mês de referência (data_entrada/saida do vínculo + created_at do participante)
--  • regra de desligado baseada em data_desligamento >= 1º dia do mês ref
DROP FUNCTION IF EXISTS public.get_participantes_turma(uuid, date);
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE tp.turma_id = _turma_id
      ...
      -- vínculo existia no mês de referência
      AND tp.data_entrada <= v_last
      AND (tp.data_saida IS NULL OR tp.data_saida >= v_first)
      -- participante existia até o fim do mês de referência
  ...
  WHERE
    CASE
      ...
      ELSE
        -- modo 'frequencia': ativos + desligados cujo desligamento ocorreu no mês ref ou depois
        b.status IN ('ativo','cadastro_incompleto','busca_ativa')
        OR (
          b.status = 'desligado'
