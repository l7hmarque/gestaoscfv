...
CREATE OR REPLACE FUNCTION public.get_dashboard_stats(_mes int DEFAULT NULL, _ano int DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_has_filter THEN
    SELECT count(*) INTO v_total_relatorios FROM relatorios_atividade WHERE data >= v_filter_start AND data < v_filter_end;
    SELECT count(*) INTO v_total_planejamentos FROM planejamentos WHERE data_aplicacao >= v_filter_start AND data_aplicacao < v_filter_end;
  ELSE
    SELECT count(*) INTO v_total_relatorios FROM relatorios_atividade;
    SELECT count(*) INTO v_total_planejamentos FROM planejamentos;
  END IF;

  -- Médias globais (filtered)
  IF v_has_filter THEN
    SELECT coalesce(avg(score_elo), 0), coalesce(avg(pct_adesao), 0)
    INTO v_media_elo, v_media_adesao
    FROM relatorios_atividade
    WHERE data >= v_filter_start AND data < v_filter_end AND (score_elo IS NOT NULL OR pct_adesao IS NOT NULL);
  ELSE
    SELECT coalesce(avg(score_elo), 0), coalesce(avg(pct_adesao), 0)
    INTO v_media_elo, v_media_adesao
    FROM relatorios_atividade
  ...
  IF v_has_filter THEN
    ...
    FROM relatorios_atividade WHERE data >= v_filter_start AND data < v_filter_end;
  ELSE
    SELECT jsonb_build_object(
      'iniciativa', round(coalesce(avg(iniciativa), 0)::numeric, 2),
      'autonomia', round(coalesce(avg(autonomia), 0)::numeric, 2),
  ...
  IF v_has_filter THEN
    ...
    FROM (
      ...
      FROM relatorios_atividade WHERE objetivo_alcancado IS NOT NULL AND data >= v_filter_start AND data < v_filter_end
      GROUP BY obj
    ) sub;
  ELSE
    SELECT coalesce(jsonb_agg(jsonb_build_object('status', obj, 'count', cnt)), '[]'::jsonb)
  ...
  IF v_has_filter THEN
    ...
    FROM (
      ...
      WHERE r.data >= v_filter_start AND r.data < v_filter_end
      GROUP BY pr.nome
      ORDER BY cnt DESC
      LIMIT 5
    ) sub;
  ...
  -- Taxa de frequência geral (filtered)
  IF v_has_filter THEN
    SELECT
    ...
    WHERE ra.data >= v_filter_start AND ra.data < v_filter_end;
  ELSE
    SELECT
  ...
  -- Atividades recentes (últimos 5 relatórios)
  ...
  FROM (
    SELECT * FROM relatorios_atividade ORDER BY data DESC, created_at DESC LIMIT 5
  ) r
  LEFT JOIN profiles pr ON pr.id = r.educador_id;
