...
BEGIN
  ...
  ELSIF _mes IS NOT NULL AND _ano IS NOT NULL THEN
    ...
    v_prev_end := v_filter_start;
  ELSE
    -- Sem filtro: usa mês corrente como janela "atual" e mês anterior como base do delta
  ...
  -- Participantes Ativos: SEMPRE = distintos com pelo menos 1 presença no período (mês corrente quando sem filtro)
  ...
  -- Distribuições e demais agregações: mantém comportamento prévio (filtro retroativo)
  ...
  SELECT coalesce(jsonb_object_agg(coalesce(bairro,'N/I'), qtd), '{}'::jsonb) INTO v_por_bairro FROM (
    SELECT bairro, count(*) as qtd FROM participantes WHERE status = 'ativo' AND is_teste = false GROUP BY bairro
  ) sub;

  SELECT coalesce(jsonb_object_agg(coalesce(periodo,'N/I'), qtd), '{}'::jsonb) INTO v_por_periodo FROM (
    SELECT periodo, count(*) as qtd FROM participantes WHERE status = 'ativo' AND is_teste = false GROUP BY periodo
  ) sub;
  ...
  -- Delta: distintos com presença no período atual vs anterior (sempre)
