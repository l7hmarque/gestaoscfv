...
CREATE OR REPLACE FUNCTION public.get_participantes_turma(_turma_id uuid, _ref_date date DEFAULT CURRENT_DATE, _modo text DEFAULT 'frequencia'::text)
 RETURNS TABLE(participante_id uuid, nome text, status text, data_desligamento date, desligado_registrado_em timestamp with time zone, busca_ativa_desde timestamp with time zone, marcador text, bloqueado_chamada boolean, bloqueado_desde date, turma_destino_nome text, data_transferencia date, vinculo_saida date)
 LANGUAGE plpgsql
